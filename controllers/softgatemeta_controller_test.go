...
func TestSoftgateMetaReconciler_DeletionWithReclaim_SkipsAPICall(t *testing.T) {
	...
	t.Log("WARNING: SoftgateMetaReconciler does not clear finalizers on deletion - potential stuck terminating state")

	updated := &k8sv1alpha1.SoftgateMeta{}
	if err := fakeClient.Get(context.Background(), req.NamespacedName, updated); err != nil {
		t.Fatalf("expected SoftgateMeta to still exist, got error: %v", err)
	}
}
